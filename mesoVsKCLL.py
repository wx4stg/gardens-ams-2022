...
if __name__ == "__main__":
    ...
    while (workingDate + timedelta(hours=14)) < endDate:
        ...
        if workingDate > gardens.index[0] and workingDate < gardens.index[-1]:
            ...
            cloudAx.set_position([cloudAx.get_position().x0, .95-cloudAx.get_position().height, cloudAx.get_position().width, cloudAx.get_position().height])
            ...
            barbsAx.barbs(workingFarmData.index, 0, workingFarmData["uwind"], workingFarmData["vwind"], barbcolor="green", flagcolor="green", alpha=0.75, label="Farm Meso")
            barbsAx.barbs(workingGardensData.index, 0, workingGardensData["uwind"], workingGardensData["vwind"], barbcolor="blue", flagcolor="blue", alpha=0.75, label="Gardens Meso")
            ...
            ax.plot(workingFarmData.index, workingFarmData["AvgAT"], "green", label="Farm Meso")
            ax.scatter(workingFarmData.index, workingFarmData["AvgAT"], s=1, c="green")
            ax.plot(workingGardensData.index, workingGardensData["AvgAT"], "blue", label="Gardens Meso")
            ...
            lax.set_position([.95-lax.get_position().width, ax.get_position().y0-.025-lax.get_position().height, lax.get_position().width, lax.get_position().height], which="both")            
